...
// Mapping represents a position mapping between .t2 and .go coordinates.
...
// TuiToGo converts a .t2 position to a .go position.
...
// GoToTui converts a .go position to a .t2 position.
// Returns the translated position and true if a mapping was found,
// or the original position and false if no mapping covers this position.
...
// FindMappingForTuiPosition finds the mapping that contains the given .t2 position.
...
// IsInGoExpression returns true if the given .t2 position is within a Go expression.
func (sm *SourceMap) IsInGoExpression(tuiLine, tuiCol int) bool {
	return sm.FindMappingForTuiPosition(tuiLine, tuiCol) != nil
}

// AllMappings returns all mappings sorted by .t2 position.
...
// Get retrieves a cached virtual file by .t2 URI.
